...
(() => {
  ...
  // XRef formatter
  const fmt = (t, f) => {
    if (f === 'full') return t.label;
    if (f === 'bare') return t.short;
    if (f === 'paren') return t.type === 'equation' ? `(${t.short})` : t.label;
    // auto
    return t.type === 'equation' ? `(${t.short})` : t.label;
  };

  // <xref to="...">
  document.querySelectorAll('xref[to]').forEach(x => {
    const id = x.getAttribute('to').replace(/^#/, '');
    const f  = x.getAttribute('format') || 'auto';
    const t  = refMap.get(id);
    if (!t) return;
    const a = document.createElement('a');
    a.className = 'xref';
    a.href = '#' + id;
    a.textContent = fmt(t, f);
    a.setAttribute('data-type', t.type);
    a.setAttribute('aria-label', t.label);
    x.replaceWith(a);
  });

  // <a class="xref" href="#..."> (or data-ref)
  document.querySelectorAll('a.xref, a[data-ref]').forEach(a => {
    let id = a.getAttribute('data-ref') || (a.getAttribute('href')||'').replace(/^#/, '');
    id = id.replace(/^#/, '');
    const f = a.getAttribute('data-format') || 'auto';
    const t = refMap.get(id);
    if (!t) return;
    a.href = '#' + id;
    if (!a.textContent.trim()) a.textContent = fmt(t, f);
    a.setAttribute('data-type', t.type);
    a.setAttribute('aria-label', t.label);
  });
})();
